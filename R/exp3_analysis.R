...
analysis_name = c("experiment3")
set.seed(21)
ccurve_file = c("data/cc_peptide_quantification.csv")
experiment_file = c("data/fragment_quantification_exp3.csv")
...
ccurve = read.csv(ccurve_file) %>% 
  ...
  mutate(Fragment.Ion = paste0(Fragment.Ion, " ", Product.Charge, "+"))#Fragment Ion does not mention charge. Add it if 2 charge states were monitored.
